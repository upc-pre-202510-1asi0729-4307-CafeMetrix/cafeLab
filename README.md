...
|--------------------|--------|-------------|--------------------------|----------------------------|
| US01 | Registro de Proveedores | Como dueño de cafetería de especialidad, quiero registrar y evaluar a mis proveedores para mantener un control de calidad y trazabilidad de origen |  |  |
| TS01 | POST HTTPS Proveedores | Como desarrollador, quiero registrar nuevos proveedores mediante una API para ampliar la red de suministro |  |  |
| TS02 | GET HTTPS Proveedores | Como desarrollador, quiero consultar proveedores mediante una API para mostrar opciones disponibles |  |  |
| US02 | Gestión de Lotes de Café Verde | Como barista profesional o dueño de cafetería de especialidad, quiero registrar y hacer seguimiento de cada lote de café verde para mantener control de inventario y trazabilidad |  |  |
| TS03 | POST HTTPS Lotes | Como desarrollador, quiero registrar nuevos lotes de café mediante una API para mantener el inventario actualizado |  |  |
| TS04 |PATCH HTTPS Lotes  |  Como desarrollador, quiero actualizar el estado de lotes mediante una API para reflejar cambios en el ciclo de vida|  |  |
| US03 |Registro de Certificaciones  |Como dueño de cafetería de especialidad, quiero documentar las certificaciones de mis cafés para validar y comunicar su valor agregado  |  |  |
...
| TS06 |GET HTTPS Certificaciones  |Como desarrollador, quiero obtener certificaciones mediante una API para mostrar valor agregado del producto  |  |  |
| US04 |Creación de Perfil de Tueste  |Como barista profesional, quiero crear perfiles de tueste personalizados para documentar y replicar mis mejores resultados  |  |  |
| TS07 |POST HTTPS Perfiles-Tueste  |Como desarrollador, quiero crear perfiles de tueste mediante una API para estandarizar procesos  |  |  |
| US05 | Registro de Sesión de Tueste |Como barista profesional, quiero documentar cada sesión de tueste asociada a un lote específico para mantener trazabilidad y control de calidad  |  |  |
| TS08 |POST HTTPS SesionesTueste  |Como desarrollador, quiero registrar sesiones de tueste mediante una API para documentar cada proceso  |  |  |
| TS09 |PATCH HTTPS SesionesTueste  |Como desarrollador, quiero actualizar lecturas de una sesión de tueste mediante una API para construir la curva real  |  |  |
| US06 |Análisis Comparativo de Tuestes  |Como barista profesional, quiero comparar diferentes sesiones de tueste para identificar patrones y optimizar resultados  |  |  |
| TS10 |GET HTTPS Análisis-Tueste  |Como desarrollador, quiero obtener análisis comparativo de tuestes mediante una API para identificar patrones  |  |  |
| US07 |Biblioteca de Defectos de Tueste  |Como barista profesional, quiero acceder a una biblioteca de defectos comunes para identificar y corregir problemas en mis tuestes  |  |  |
...
| US08 |Cata Digital Estructurada  |Como barista profesional, quiero registrar evaluaciones sensoriales estructuradas para documentar las características de cada lote y tueste  |  |  |
| TS12 |POST HTTPS Catas  |Como desarrollador, quiero registrar evaluaciones sensoriales mediante una API para documentar características del café  |  |  |
| US09 |Visualización de Perfiles Sensoriales  |Como barista profesional o dueño de cafetería de especialidad, quiero visualizar perfiles sensoriales en formato de hexágono o gráfico de radar para interpretar y comparar cualidades  |  |  |
| TS13 |GET HTTPS PerfilesSensoriales  |Como desarrollador, quiero obtener perfiles sensoriales mediante una API para visualizar y comparar cualidades  |  |  |
| US10 |Correlación Tueste-Sabor  |Como barista profesional, quiero visualizar la correlación entre parámetros de tueste y resultados sensoriales para optimizar mis perfiles  |  |  |
| TS14 |GET HTTPS Correlaciones  |Como desarrollador, quiero obtener correlaciones entre tueste y sabor mediante una API para optimizar procesos  |  |  |
| US11 |Historial de Catas  |Como barista profesional, quiero acceder al historial de catas por lote, origen o periodo para analizar tendencias y consistencia  |  |  |
| US12 |Creación de Recetas de Preparación  |Como barista profesional, quiero crear y documentar recetas detalladas para cada método de preparación y tipo de café  |  |  |
| TS15 |POST HTTPS Recetas  |Como desarrollador, quiero registrar recetas de preparación mediante una API para documentar métodos óptimos  |  |  |
| US13 |Calibración de Molienda  |Como barista profesional, quiero documentar configuraciones de molienda para diferentes equipos y métodos para mantener consistencia entre preparaciones  |  |  |
| TS16 |POST HTTPS Calibraciones  |Como desarrollador, quiero registrar calibraciones de molienda mediante una API para mantener consistencia entre preparaciones  |  |  |
| US14 |Portafolio de Bebidas  |Como barista profesional o dueño de cafetería de especialidad, quiero crear un portafolio digital de bebidas y recetas para presentar a clientes o eventos  |  |  |
| US15 |Compartir Recetas  |Como barista profesional, quiero compartir mis recetas con mi equipo para mantener consistencia en la preparación  |  |  |
| US16 |Control de Inventario Integrado  |Como dueño de cafetería de especialidad, quiero gestionar el inventario de café verde y tostado de forma integrada para optimizar recursos y prevenir desabastecimiento  |  |  |
| TS17 |GET HTTPS Inventario  |Como desarrollador, quiero consultar el inventario integrado mediante una API para visualizar existencias  |  |  |
...
| US17 |Gestión de Costos de Producción  |Como dueño de cafetería de especialidad, quiero registrar y analizar los costos de producción de cada lote para optimizar mi estructura de precios y rentabilidad  |  |  |
| US18 |Análisis de Eficiencia y Rendimiento  |Como dueño de cafetería de especialidad, quiero monitorear y comparar el rendimiento productivo entre distintos lotes para identificar factores que afectan la eficiencia  |  |  |
| US19 |Reportes de Trazabilidad  |Como dueño de cafetería de especialidad, quiero generar reportes de trazabilidad completa para comunicar transparencia y valor agregado  |  |  |
| US20 |Información del Producto  |Como visitante de la landing page, quiero encontrar información clara sobre la plataforma para entender sus beneficios y decidir si me interesa  |  |  |
| US21 |Secciones Específicas por Segmento  |Como visitante de la landing page, quiero encontrar información adaptada a mi perfil profesional para evaluar si la solución responde a mis necesidades específicas  |  |  |
| US22 |Contacto con Equipo  |Como visitante de la landing page, quiero contactar con el equipo del sistema para resolver dudas específicas antes de registrarme  |  |  |
| US23 |Acceso directo a plataforma  |Como visitante de la landing page, quiero redirigirme hacia la aplicación principal desde la landing page para comenzar a usar el sistema inmediatamente  |  |  |
| TS19 |POST HTTPS Contacto  |Como desarrollador, quiero procesar formularios de contacto mediante una API para gestionar comunicaciones con visitantes  |  |  |
| US24 |Registro y Autenticación (Usuarios)  |Como barista profesional o dueño de cafetería de especialidad, quiero registrarme y acceder de forma segura para mantener la confidencialidad de mis datos  |  |  |
| TS20 |POST HTTPS Usuarios  |Como desarrollador, quiero registrar nuevos usuarios mediante una API para permitir el acceso al sistema  |  |  |
| TS21 |- POST HTTPS Autenticación  |Como desarrollador, quiero autenticar usuarios mediante una API para validar credenciales y permitir acceso seguro  |  |  |
| US25|Perfil Personalizado  |Como barista profesional o dueño de cafetería de especialidad, quiero configurar mi perfil profesional para personalizar mi experiencia y mostrar mi identidad dentro del sistema  |  |  |
| US26 |Selección de Plan  |Como barista profesional o dueño de cafetería de especialidad, quiero elegir entre los diferentes planes disponibles para acceder a las funcionalidades que mejor se adapten a mis necesidades  |  |  |
| TS22 |GET HTTPS Planes  |Como desarrollador, quiero obtener información sobre planes disponibles mediante una API para mostrar opciones de suscripción  |  |  |
| TS23 |POST HTTPS Planes  |Como desarrollador, quiero suscribir usuarios a planes mediante una API para gestionar acceso a funcionalidades  |  |  |
